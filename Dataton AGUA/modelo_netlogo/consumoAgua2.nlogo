...
turtles-own[poblacion tcd tca ];;tca puede valer 1,2,3  tcd=tasa de crecimiento deseada tca=tasa de consumo de agua
...
to ajusta-fuente
  ask fuente[
    ...
    ask salidas[
      set flujo oferta / cuenta-salidas
    ]
    ask out-flecha-neighbors[ ;; out son las flechas que salen al nodo actual
      recorre-red
    ]
...
to recorre-red ;;primero en profundidad
...
to actualiza-poblacion
  ...
  let tcr 0
  ifelse demanda > mi-oferta[
    set tcr tcd * demanda / mi-oferta
  ][
    set tcr tcd; si satisface o excede la demanda
  ]
  set poblacion int  poblacion * tcr
   if poblacion > k [set poblacion k]
  if poblacion < 1 [set poblacion 1
  ]
  set size poblacion / 100
